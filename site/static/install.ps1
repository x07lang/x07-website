...
if ($Json) {
  ...
  Write-Host $pathHint
  Write-Host ""
  Write-Host "next: create a project (includes agent kit: AGENT.md + .agent/{docs,skills})"
  Write-Host "  mkdir myapp; cd myapp; x07 init"
  Write-Host "  x07 run"
  Write-Host "  x07 test --manifest tests/tests.json"
  Write-Host ""
  Write-Host "next: open offline docs / check skills"
  Write-Host "  x07up docs path"
  Write-Host "  x07up skills status --json"
  Write-Host ""
