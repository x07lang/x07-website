...
5. `x07 xtal tests gen-from-spec --project x07.json --write`
6. `x07 test --all --no-fail-fast --manifest gen/xtal/tests.json`
7. `x07 xtal dev --project x07.json`
...
Use `--all` when running generated XTAL tests manually so `ensures_props` property wrappers run with the example tests.
