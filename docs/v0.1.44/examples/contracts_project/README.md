...
2. Ensure your repo contains the modules referenced by these contracts (typically via vendored deps under `.x07/deps/`).
3. Run `x07 arch check --write-lock`.
4. Commit:
...
- Archive `ops_allowed` symbols are validated against module exports found under your repo root (including vendored deps under `.x07/deps/`).
