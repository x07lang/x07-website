...
This spec defines a deterministic, file-backed module system and a minimal package+lockfile workflow for reproducible builds.
...
This enables a standalone binding pattern: `std.fs` stays source-stable while the world selects the underlying adapter.
...
Example project: `docs/examples/agent-gate/web-crawler-local/`.
