...
This aligns with your memory and borrow-instrumentation goals.
...
* no explosion of core semantics while the core surface is still stabilizing

### Option 2 (next): add **ViewBytes** as a distinct core type
...
This aligns with your view/borrow goals.
...
pub enum x07Ty {
    ...
    // Bytes
    Bytes,      // owning buffer
    BytesView,  // borrowed view (ptr+len concept; may still lower to Bytes until `bytes_view` is fully represented at runtime)
...
fn validate_rust_subset(src: &str) -> Result<(), Vec<Diag>> {
    ...
    // Step 1: collect signatures deterministically (source order).
    v.collect_fn_sigs(&file);

    // Step 2: validate bodies via visitor.
    v.visit_file(&file);
    ...
    fn validate_fn_sig_only(&mut self, sig: &syn::Signature) -> bool {
        ...
        if sig.asyncness.is_some() {
            ...
            self.push("X7I_FN_ASYNC", span(sig),
                ...
                Some("use a synchronous function; async is not part of the supported v1 subset".into()));
        }
