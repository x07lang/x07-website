...
- `solve-fs`: deterministic read-only fixture filesystem mounted as current directory (`.`).
  ...
  - Deterministic streaming adapters:
...
- `solve-kv`: deterministic key/value store (seeded per case; reset per case).
  ...
  - Latency index: runner may also materialize `./.x07_kv/latency.evkvlat` (used to advance virtual time for `kv.get`).
...
Standalone OS worlds include OS builtins (compile-time gated to `run-os*`):
...
Standalone OS worlds also support unsafe + FFI (compile-time gated to `run-os*`):
