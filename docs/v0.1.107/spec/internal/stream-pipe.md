...
- JSON canonicalization (`std.stream.xf.json_canon_stream_v1`) is implemented as a stream plugin (`xf.json_canon_stream_v1`) in the `x07.stream.xf` native backend, and uses the built-in C JSON JCS runtime section emitted by the x07c C backend (`crates/x07c/src/c_emit_*.rs`).
